...
In the following we describe the learning behaviour of the two player classes according to changes in parameters (learning rate and randomization). 

In order to study the behaviour of the algorithm we let two robot players play against each other. We observe the winning and draw game rates for an increasing number of training games. For both players we record over time how the value of a selection of possible starting states evolves over time. We chose particular values of the starting positions for two reasons. First, these positions are reached frequently when playing the game again and again, since in the beginning of the game there are only a limited number of possible scenarios. Consequently, the values of these states are updated frequently and more likely to depict the learning behavior and success of the two players. Second, the evolution of these values illustrates well that while the agents make greedy moves - taking into account only the immediate consequence of actions - the algorithm still achieves strategies which are overall “optimal”. 

First we examine the specification when both players have the same learning parameter value. We exclude random choices and let both players decide optimally in a greedy way.

PLOTS PLOTS PLOTS

Plot XXX to YYY depict the outcome of games as a function of the number of training games. In general we can observe that the number of shares of draws is increasing with more games. For lower learning rates the share of draw games is increasing faster. Furthermore, the development for higher levels of $\alpha$ is less volatile. 

It can be seen that the value of most of the states does not change at all for both players. With no randomization both players are stuck in a given strategy (a local optimum) which leads to a first player winning most of the times while the second player does not find the strategy to beat the first player. In all cases the winning share of game opening player is constantly higher, which makes sense since he has a slight advantage. 

Now we observe what happens with different learning rates if the players make exploratory moves. We set the level of randomization to $\epsilon = 0.02$ - we tried different levels of randomisation and this seems to be a reasonable level for the learning behaviour. The final results of the games looks quite similar as in the no radomisation case. But, as with randomisation, both players explore more strategies, the values of the starting positions are constantly updated.  

PLOTS

The first player still performs better in general, but different first moves are distinguished. For the first player, starting in the middle (green dashed line) seems to be slightly better than any other starting point. For the second player,  states which are available after the first player started at the center are in general disadvantageous. This is not surprising since this is the winning strategy of the first player, according to our results. On the other hand, the second player also learned for example that if the first player starts at the corner it is better to play in the center (brown solid line) than to put next to the first player (green solid line).  Note that the robot trained with no randomization might performed similarly against the other robot player even if it did not have this extra information.  But when the robot has to play against a human, who does not necessarily play the optimal strategy, this information might be very useful. 
